...
keytool -export -keystore info.p12 -alias info -file infoCert.crt
keytool -importcert -file infoCert.crt -alias info -keystore certs.p12 -storepass changeit

:: Import cert from status
keytool -export -keystore status.p12 -alias status -file statusCert.crt
keytool -importcert -file statusCert.crt -alias status -keystore certs.p12 -storepass changeit